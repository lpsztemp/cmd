...
echo define(REPLACE_HTML, `patsubst(patsubst(patsubst(patsubst(patsubst(patsubst(patsubst(````````$*'''''''', `^<', `^&lt;'), `^>', `^&gt;'), ` +', `^&nbsp;'), `:', `^&#58;'), `~', `^&#126;'), `,(\w)', `, \1'), `\\n', `^<BR/^>')')dnl >> "%M4_OUTPUT%"
echo define(HIGHLIGHT_COMMENTS, `patsubst(`$*', `\(/\*.*?\*/\)', `^<FONT COLOR="green4"^>^<I^>\1^</I^>^</FONT^>')')dnl >> "%M4_OUTPUT%"
...
echo define(HIGHLIGHT_KEYWORDS, `HIGHLIGHT_TAGS(``$*'', `virtual', `override', `final', `class', `template', `const', `volatile', `struct', `int', `long', `short', `char', `double', `float', `bool', `true', `false', `auto', `typedef', `noexcept', `enum', `constexpr', `signed', `unsigned', `operator', `explicit', `nullptr', `void')')dnl >> "%M4_OUTPUT%"
...
echo define(CLASS_MEMBER, `	^<TR^>^<TD align="left" PORT="'`PORT_NAME(CURRENT_CLASS_NAME(),`$*')'`"^>^&nbsp;^&nbsp;^&nbsp;'`HIGHLIGHT_COMMENTS(HIGHLIGHT_KEYWORDS(REPLACE_HTML(`$*')))'`^</TD^>^</TR^>')dnl  >> "%M4_OUTPUT%"
